...
{
    0, 55, 96, 169, 192, 290, 342, 386, 456, 490, 567, 585,
};

Canvas::Canvas()
{
    SDL_Init(SDL_INIT_VIDEO); IMG_Init(IMG_INIT_PNG);
    Win = SDL_CreateWindow("NVplayer++", 0, 0, 1920, 840, 0);
...
void Canvas::Note(int k, int yb, int ye, color c)
{
    ...
    for (int _x = x + 1; _x <= x_; ++_x, c *= 1.02)
    {
        const RGBA_pix &p = getColor(c);
        SDL_SetRenderDrawColor(Ren, p.r, p.g, p.b, p.a);
    ...
    if (yb - ye > 4)
    {
        ...
        SDL_Rect R{x, ye, w + 2, h}; SDL_RenderDrawRect(Ren, &R);
    }
}
